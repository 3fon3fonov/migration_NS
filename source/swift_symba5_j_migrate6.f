...
c  allows planet i's Jacobi semi-major axis and eccentricity to
c  change at rates dadt(i)/sqrt(a) and dedt(i)*e/a**1.5.
c
...
c Last revision: 
      ...
      real*8 dadt(NTPMAX),dedt(NTPMAX)
      integer ialphai
...
c Prompt and read rates of change of semi-major axes and eccentricities.
	write(*,*)
     &     'Enter rates of change of semi-major axis and eccentricity :'
	do i=2,nbod
	  read(*,*) dadt(i),dedt(i)
	  write(*,*) ' i,dadt,dedt = ',i,dadt(i),dedt(i)
	enddo
...
c Update each planet's semi-major axis and ecc. by half dt
	 gmi = mass(1)
         do i=2,nbod
            gmi = gmi + mass(i)
     ...
     &           gmi,ialphai,ai,ei,inci,capomi,omegai,capmi)
            ai = ai*(1.d0 +
     &               0.75d0*dt*dadt(i)/ai**1.5d0)**(1.d0/1.5d0)
            ei = ei*exp(0.5d0*dt*dedt(i)/ai**1.5d0)
